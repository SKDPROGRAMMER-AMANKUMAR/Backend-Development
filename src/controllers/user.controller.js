...
// Cloudinary Upload Function with Image Replacement Logic
...
//     return result; // Return Cloudinary response (URL, public_id, etc.)
//   } catch (error) {
//     throw new Error(`Cloudinary Upload Error: ${error.message}`);
//   }
// }; ---->> so it means , we don't need to write the "uploadOnCloudinary" before the main controller , cause here we simple use the exported function that we export it from the cloudinary.js file which is "uploadOnCloudinary" , is this right ??

const updateUserAvatar = asyncHandler(async(req, res) => {
   const avatarLocalPath = req.file?.path/*You can save it directly to the database(MongoDB) without using cloudinary
   
   req.file:
  In Express, when handling file uploads using middleware like multer, req.file contains information about a single uploaded file.
  It stores metadata such as the file's original name, MIME type, and the path where the file was    temporarily saved.

  ?. (Optional Chaining):   
  This ensures that the code does not throw an error if req.file is undefined.
  If req.file exists, it accesses the path property; otherwise, it returns undefined safely without crashing.

  path Property:  
  The path contains the local file system path where the uploaded file is stored. This is generated by the multer middleware, which temporarily saves the uploaded file to a specific directory.
...
*/
  ...
  if (!updatedUser) {
   ...
   // Extract public ID from the existing avatar URL (if available)
  const publicId = user.avatar
  ? user.avatar.split("/").pop().split(".")[0]
  : null;

  //  Todo: write the utility function to delete previous image . 

   const avatar = await uploadOnCloudinary(avatarLocalPath,publicId)/*avatarLocalPath:
   This is the local file path of the uploaded avatar image, assigned earlier as:
   const avatarLocalPath = req.file?.path;
   
   uploadOnCloudinary:
   It handles the upload of a local file (given by avatarLocalPath) to the Cloudinary server.
   This function is likely a custom wrapper for Cloudinary’s API.
   The function uses Cloudinary's SDK to send the file, possibly along with additional configurations (e.g., folder paths or transformations).

   avatar:
   Public URL of the uploaded image.
   Secure URL for HTTPS access.

   Once the upload is successful, avatar stores the Cloudinary response. This response might include:
   Metadata like the size, format, and transformation data.
   Public ID used for accessing the media later.
   */
   ...
   const user = await User.findByIdAndUpdate(/*User.findByIdAndUpdate:
    Finds the user by their ID and updates the document in the database.
    This method is part of Mongoose, a MongoDB object modeling tool for Node.js.*/
       req.user?._id,/*req.user?._id:
       Extracts the user’s ID from the authenticated request object (req.user).
       The optional chaining (?.) ensures the code won’t throw an error if req.user is undefined.*/
       {
        $set:{
          avatar:avatar.url
        }/*$set Operator:
        This MongoDB operator updates only the specified fields (here, the avatar field).
        avatar: avatar.url sets the new avatar URL (received from Cloudinary).*/
       },
       {new:true}/*{ new: true }:
       This option ensures that the method returns the updated user document rather than the original.*/
   ).select("-password")/*.select("-password"):
   This excludes the password field from the user document returned to the caller, for security purposes.*/
